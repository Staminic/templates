...
// Set default panel
if ($panel == '' && in_array($widget->position, array('top-hero', 'top-a', 'top-b', 'top-c', 'top-d', 'bottom-a', 'bottom-b', 'bottom-c', 'bottom-d', 'bottom-hero', 'main-top', 'main-bottom', 'sidebar-a', 'sidebar-b'))) {
    $panel = $this['config']->get("panel_default.{$widget->position}.panel", '');
}
// Set panel for specific positions
else if (in_array($widget->position, array('headerbar', 'toolbar-r' ,'toolbar-l', 'footer', 'offcanvas', 'offcanvas-right'))) {
    $panel = 'uk-panel';
}
...
if ($widget->menu) {
    ...
    if (isset($params['menu'])) {
        ...
        $widget->nav_settings["dropdown"] = array("remaintime" => 400);
    } else if (in_array($widget->position, array('toolbar-l', 'toolbar-r', 'footer'))) {
        $renderer = 'subnav';
        $widget->nav_settings["modifier"] = "uk-subnav-line";
    } else if (in_array($widget->position, array('offcanvas', 'offcanvas-right'))) {
        $renderer = 'nav';
